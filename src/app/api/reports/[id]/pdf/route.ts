 ...
 * Returns a true PDF binary generated by headless Chromium (puppeteer-core +
 * @sparticuz/chromium-min).  Falls back to print-ready HTML if Chromium is
 * unavailable (local dev without the binary).
 *
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // ── Attempt real PDF via headless Chromium ──────────────────────────────
  try {
    const chromium = await import("@sparticuz/chromium-min");
    const puppeteer = await import("puppeteer-core");

    const executablePath = await chromium.default.executablePath(
      // In production (Vercel) the binary is fetched from the CDN package.
      // In local dev set CHROMIUM_EXECUTABLE_PATH to skip the download.
    ...
    // Chromium unavailable (local dev) — fall back to HTML download
    console.warn("[pdf] Chromium unavailable, falling back to HTML:", (pdfErr as Error).message);
