...
/**
 * POST /api/reports/[id]/generate-infographic
 * User-triggered generation for a single paid analysis infographic.
 */
export async function POST(
  ...
  try {
    ...
    if (!isManualPaidInfographicSection(body.section)) {
    ...
    if (asset?.status === "pending") {
      return NextResponse.json({ ok: true, skipped: true, reason: "already_pending" });
    }

    kickOffInfographicSectionInBackground(id, body.section, false);
    return NextResponse.json({ ok: true, generating: true, section: body.section });
  } catch (err) {
    console.error("[POST /api/reports/[id]/generate-infographic]", err);
