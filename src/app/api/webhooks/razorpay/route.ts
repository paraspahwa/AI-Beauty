...
export async function POST(req: NextRequest) {
  ...
  if (event.event === "payment.captured") {
    ...
    // Fire-and-forget: generate previews after unlock.
    if (row.report_id) {
      const internalSecret = process.env.INTERNAL_API_SECRET;
      const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://renovaara.in";
      if (internalSecret) {
        fetch(`${appUrl}/api/internal/trigger-previews`, {
          method: "POST",
          headers: {
        ...
        }).catch((e) => console.error("[webhook/razorpay] trigger-previews fire failed", e));
      }
    }
