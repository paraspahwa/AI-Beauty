...
export function HairstyleCard({ data }: { data: HairstyleResult }) {
  return (
    <Card>
      <CardHeader>
        <p className="text-xs uppercase tracking-widest text-ink-muted">Hairstyle Guide</p>
        <CardTitle>Cuts &amp; colors made for you</CardTitle>
      </CardHeader>
      <CardContent className="space-y-8">
        <section>
          <h4 className="mb-3 text-xs uppercase tracking-widest text-ink-muted">Recommended styles</h4>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {data.styles.map((s) => (
              <div key={s.name} className="rounded-2xl border border-cream-200 bg-cream-100 p-4">
                ...
                <p className="mt-1 text-sm text-ink-muted leading-snug">{s.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h4 className="mb-3 text-xs uppercase tracking-widest text-ink-muted">Length guide</h4>
          <div className="grid gap-3 sm:grid-cols-3">
            {data.lengths.map((l) => (
              <div key={l.name} className="rounded-2xl border border-cream-200 bg-white/70 p-4 text-center">
                ...
                <p className="mt-1 text-xs text-ink-muted">{l.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h4 className="mb-3 text-xs uppercase tracking-widest text-ink-muted">Color suggestions</h4>
