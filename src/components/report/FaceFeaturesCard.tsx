...
// -- Icons ù thin, elegant line art exactly matching reference -----------------
function IconFace() {
  return (
...
function FeatureBox({
  ...
  // Split on period or semicolon ù commas inside notes stay intact
  const bullets = notes
    .split(/[.;]/)
    .map((s) => s.trim())
    .filter(Boolean)
  ...
  return (
    ...
    >
      ...
      <div className={`grid gap-8 px-4 sm:px-6 md:px-10 pb-10 items-center ${showFeatures ? "grid-cols-1 md:grid-cols-[1fr_auto_1fr]" : "grid-cols-1 max-w-md mx-auto"}`}>
        ...
        {/* Left column: Face Shape ù Eyes ù Nose */}
        ...
        {/* Right column: Eyebrows ù Cheeks ù Lips */}
        {showFeatures && features ? (
          <div className="flex flex-col gap-4">
            <FeatureBox
              featureKey="eyebrows"
